...
export const studentsTable = table(
  "students",
  {
    id: id(),
    ...
    behavioralFlags: jsonText("behavioral_flags")
      .$type<string[]>()
      .notNull()
      .$defaultFn(() => []),
    notes: text("notes"),
    ...
    privateTipUpdatedBy: integer("private_tip_updated_by").references(
      () => usersTable.id,
      { onDelete: "set null" },
    ),
    status: text("status", {
      enum: ["active", "stopped", "graduated", "deleted"],
    })
      .notNull()
      .default("active"),
