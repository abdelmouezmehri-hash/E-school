...
Kidspeak LMS supports a **multi-branch model** where a single school instance can manage multiple physical or logical branches, each with isolated data visibility for branch managers while preserving global oversight for admins.

This architecture was added in **Phase 2** (May 2026) and lays the foundation for future SaaS expansion to multiple schools.

### 15.2 Core Concepts

| Concept | Description |
|---|---|
| **Branch** | A physical or logical location of the school (e.g., "Sétif Main", "Algiers North") |
| **Branch Manager** | A role with visibility limited to a single branch's data |
| **Admin** | A role with full visibility across all branches (default) |
| **branchId** | Foreign key column on data tables, references `branches.id` |
| **Branch Inheritance** | When records are created, `branchId` is auto-inherited from related entities |

### 15.3 Database Schema

The following 11 tables have a `branchId INTEGER REFERENCES branches.id ON DELETE SET NULL` column:

| Table | Inheritance Source |
|---|---|
| `payments` | From student |
| `paymentTransactions` | From parent payment |
| `evaluations` | From student |
| `observations` | From student |
| `consultations` | From student (if linked) |
| `enrollmentRequests` | Set by parent or admin |
| `registrationRequests` | Set by admin upon approval |
| `staffPaymentRequests` | From staff user |
| `news` (school_news) | From author user |
| `media` | From student or group |
| `levels` | Set explicitly (admin/branch_manager) |

**Design choices:**
- All `branchId` columns are **nullable** (`NULL` = "global" or "unassigned")
- `ON DELETE SET NULL` — deleting a branch does NOT cascade to data deletion
- Backward compatible — existing data was backfilled (see Section 15.7)

### 15.4 Branch Helper Functions

Located at `lib/db/src/helpers/branchScope.ts`, exported from `@workspace/db`.

```typescript
// Lookup utilities — used in routes when creating new records
getBranchIdFromStudent(studentId: number): Promise<number | null>
getBranchIdFromGroup(groupId: number): Promise<number | null>
getBranchIdFromUser(userId: number): Promise<number | null>

// Query scoping — used to filter results based on user role
getBranchScope(userRole: string, userBranchId: number | null): number | null
// Returns:
//   null  if admin (sees all branches)
//   N     if branch_manager (sees only branch N)
//   -1    if branch_manager has no branch (defensive: empty result)

// Type guard
isBranchManager(user: { role?: string }): boolean
```

**Usage example in a route:**

```typescript
import { getBranchIdFromStudent } from "@workspace/db";

// Auto-inherit branchId from the student
const inheritedBranchId = await getBranchIdFromStudent(parsed.data.studentId);
...
### 15.5 Role-Based Access Pattern

| Role | Visibility | Write Access |
|---|---|---|
| `admin` | All branches | All branches |
| `branch_manager` | Own branch only | Own branch only |
| `teacher` | Assigned groups only | Own work only |
| `parent` | Own children only | None (read-only mostly) |
| `psychologist` | All students | Own consultations/observations |

### 15.6 Branch Filter UI

The frontend exposes a **Branch Filter** in the sidebar (`/branches` page and global context):

- **Admins** see "All Branches" option + dropdown of all branches
- **Branch managers** see only their assigned branch (filter is locked)
- The selected branch propagates via `BranchProvider` React context

### 15.7 Phase 2 Migration History

#### Phase 2.1 — Schema Update (Commit `89fb7da`)
Added `branchId` column to 11 tables. Migration applied via:
```bash
pnpm -F @workspace/db push:dev   # SQLite (dev)
pnpm -F @workspace/db push:prod  # PostgreSQL (production)
```

#### Phase 2.2 — Branch Helpers (Commit `b6987bf`)
Created `lib/db/src/helpers/branchScope.ts` with 5 helper functions and exported from `@workspace/db`.

**Status:** Helpers are available globally. Their integration into routes is incremental — each new feature should use them when creating records that should be branch-scoped.

#### Phase 2.4 — Backfill (DB-only, no code commit)
Created the first branch ("Main Branch") and assigned all legacy records to it:

```sql
-- Step 1: Create Main Branch
INSERT INTO branches (name, name_ar, address, phone)
VALUES ('Main Branch', 'الفرع الرئيسي', 'Sétif, Algeria', '+213000000000');

-- Step 2: Assign all existing students/users/groups to branch 1
UPDATE students SET branch_id = 1 WHERE branch_id IS NULL;
UPDATE users    SET branch_id = 1 WHERE branch_id IS NULL;
UPDATE groups   SET branch_id = 1 WHERE branch_id IS NULL;

-- Step 3: Cascade to dependent tables
UPDATE payments p   SET branch_id = s.branch_id FROM students s WHERE p.student_id = s.id   AND p.branch_id IS NULL;
UPDATE evaluations e SET branch_id = s.branch_id FROM students s WHERE e.student_id = s.id  AND e.branch_id IS NULL;
-- ... (similar for observations, consultations, media, etc.)
```

**Result after backfill:**
| Table | Records | With Branch |
|---|---|---|
| students | 2 | 2 ✅ |
| users | 13 | 13 ✅ |
| groups | 2 | 2 ✅ |
...
- [ ] **Phase 2.3** — Frontend `BranchProvider` integration; admin can switch active branch
- [ ] **Phase 2.5** — Create `branch_manager` test user; verify isolation
- [ ] **Phase 2.6** — Apply `getBranchIdFromStudent` to all 8 backend routes that create student-related records (payments, evaluations, observations, media, consultations)
- [ ] **Phase 2.7** — Add branch-scoped query filters to GET endpoints (currently most routes return all data; branch_managers should see only their branch)

### 15.9 Operational Notes

**When adding a new feature with branch-scoped data:**

1. If the new entity belongs to a student/group, use `getBranchIdFrom*()` to auto-inherit
2. If the entity is created by an admin, allow them to specify the branch via the UI
3. Always use `getBranchScope(user.role, user.branchId)` in GET endpoints to filter results
4. Test with both `admin` and `branch_manager` accounts

**Common pitfalls:**

- ⚠️ Using `pnpm -F @workspace/db push:dev` only updates SQLite — production runs PostgreSQL, requires `push:prod` too
- ⚠️ Replit Shell HTML-converts text resembling domains (e.g., `usersTable.id`) — use Replit Editor (GUI) or Base64 for code edits
- ⚠️ `git apply` patches often fail due to whitespace; prefer manual edits or `sed` with simple patterns
