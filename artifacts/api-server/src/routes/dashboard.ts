...
const requireDashboardRole = await requireRole([
  "admin",
  "accountant",
  "branch_manager",
]);
  ...
  async (_req: Request, res: Response): Promise<void> => {
    const [
          ...
          or(
            ...
            and(
              or(
                ...
                eq(paymentsTable.status, "partially_paid"),
              ),
      ...
      db
        .select({ count: count() })
        .from(studentsTable)
        .where(eq(studentsTable.status, "stopped")),
      db
        .select({ count: count() })
        .from(studentsTable)
        .where(eq(studentsTable.status, "graduated")),
      db
        .select({ count: count() })
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    const [totals, revenueByLevel, statusCounts, [expenseTotal]] =
      await Promise.all([
    ...
    const statusBreakdown = statusCounts[0] ?? {
      paid: 0,
      partially_paid: 0,
      overdue: 0,
      pending: 0,
...
router.get(
  ...
  async (_req: Request, res: Response): Promise<void> => {
    ...
    for (const s of students) {
      ...
      if (s.behavioralFlags.includes("high_potential"))
        flagCounts.high_potential++;
    }
  ...
  async (_req: Request, res: Response): Promise<void> => {
    ...
    const result = students.map((s) => {
      ...
      return {
        ...
        studentName: s.name,
        levelName: s.levelId
          ? (levelsMap.find((l) => l.id === s.levelId)?.name ?? null)
          : null,
        ...
        observations: observations
          ...
          .map((o) => ({
            ...o,
            authorName: o.authorName ?? "Unknown",
  ...
  async (_req: Request, res: Response): Promise<void> => {
    ...
    const result = payments.map((p) => {
      ...
      return {
        ...
        studentName:
          studentsMap.find((s) => s.id === p.studentId)?.name ?? "Unknown",
        levelName: p.levelId
          ? (levelsMap.find((l) => l.id === p.levelId)?.name ?? null)
          : null,
        amountDue,
        amountPaid,
