...
import {
  ...
  branchesTable,
  adhocSessionsTable,
  confidenceMetricsTable,
  performanceReportsTable,
  marketingEnrollmentRequestsTable,
  leadsTable,
} from "@workspace/db";
import {
  CreateStudentBody,
...
function calcProgressScore(
  ...
  if (evals.length < 2)
    return evals.length === 1
      ? ((evals[0].speakingScore +
          evals[0].confidenceScore +
          evals[0].participationScore) /
          3) *
          10
      : null;
  ...
  const first =
    ...
    3;
  const last =
    (evals[evals.length - 1].speakingScore +
      evals[evals.length - 1].confidenceScore +
      evals[evals.length - 1].participationScore) /
    3;
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    const forceBranchId =
      user.role === "branch_manager" ? (user.branchId ?? -1) : null;
    ...
    if (params.success) {
      ...
      if (params.data.levelId) {
        ...
        students = await db
          ...
          .where(
            forceParentId
              ? and(levelCond, eq(studentsTable.parentId, forceParentId))
              : levelCond,
          )
          .orderBy(studentsTable.name);
      }
      ...
      if (params.data.search) {
        ...
        students = await db
          ...
          .where(
            forceParentId
              ? and(searchCond, eq(studentsTable.parentId, forceParentId))
              : searchCond,
          )
          .orderBy(studentsTable.name);
      }
    ...
    const levelIds = [
      ...new Set(
        students.map((s) => s.levelId).filter((id): id is number => id != null),
      ),
    ];
    const parentIds = [
      ...new Set(
    ...
    if (levelIds.length > 0) {
      ...
      levels.forEach((l) => levelsMap.set(l.id, l.name));
    }
    ...
    if (studentIds.length > 0) {
      ...
      payments.forEach((p) => {
        if (!paymentsMap.has(p.studentId))
          paymentsMap.set(p.studentId, p.status);
      });
    }
    ...
    if (studentIds.length > 0) {
      const evals = await db
        ...
        .where(inArray(evaluationsTable.studentId, studentIds))
        .orderBy(evaluationsTable.weekNumber);
      evals.forEach((e) => {
        if (!evalsMap.has(e.studentId)) evalsMap.set(e.studentId, []);
        evalsMap.get(e.studentId)!.push(e);
      });
    }

    const groupMap = new Map<
      number,
      { groupId: number; groupName: string; levelId: number | null }
    >();
    if (studentIds.length > 0) {
      const groups = await db
        .select({
          ...
          groupLevelId: groupsTable.levelId,
        })
      ...
      groups.forEach((g) => {
        if (!groupMap.has(g.studentId)) {
          groupMap.set(g.studentId, {
            groupId: g.groupId,
            groupName: g.groupName,
            levelId: g.groupLevelId ?? null,
          });
        }
      });
    }

    // ── Sessions per group: completed (status != "planned") and total expected ──
    // completed = real class_sessions rows with status != "planned"
    // total = level.durationWeeks * level.sessionsPerWeek (falls back to count of group's sessions)
    const groupIds = [
      ...new Set(Array.from(groupMap.values()).map((g) => g.groupId)),
    ];
    const completedByGroup = new Map<number, number>();
    const allByGroup = new Map<number, number>();
    if (groupIds.length > 0) {
      ...
      for (const r of sessionRows) {
        allByGroup.set(r.groupId, (allByGroup.get(r.groupId) ?? 0) + 1);
        if ((r.status ?? "completed") !== "planned") {
          completedByGroup.set(
            r.groupId,
            (completedByGroup.get(r.groupId) ?? 0) + 1,
          );
        }
      }
    }
    ...
    if (groupLevelIds.length > 0) {
      const lvls = await db
        ...
        .from(levelsTable)
        .where(inArray(levelsTable.id, groupLevelIds));
      lvls.forEach((l) =>
        levelMetaMap.set(l.id, {
    ...
    const enriched = students.map((student) => {
      const group = groupMap.get(student.id);
      const completedSessions = group
        ? (completedByGroup.get(group.groupId) ?? 0)
        : 0;
      const lvlMeta =
        group?.levelId != null ? levelMetaMap.get(group.levelId) : undefined;
      const expectedFromLevel = lvlMeta
        ? lvlMeta.durationWeeks * lvlMeta.sessionsPerWeek
        : 0;
      const totalSessions = group
        ? Math.max(expectedFromLevel, allByGroup.get(group.groupId) ?? 0)
        : 0;
      return {
        ...student,
        createdAt: student.createdAt.toISOString(),
        levelName: student.levelId
          ? (levelsMap.get(student.levelId) ?? null)
          : null,
        parentName: student.parentId
          ? (usersMap.get(student.parentId) ?? null)
          : null,
        teacherName: student.teacherId
          ? (usersMap.get(student.teacherId) ?? null)
          : null,
        paymentStatus: paymentsMap.get(student.id) ?? null,
        latestProgressScore: calcProgressScore(evalsMap.get(student.id) ?? []),
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    const [student] = await db
      ...
      .values({
        ...
        enrollmentDate:
          parsed.data.enrollmentDate instanceof Date
            ? parsed.data.enrollmentDate.toISOString().split("T")[0]
            : String(parsed.data.enrollmentDate),
        behavioralFlags: parsed.data.behavioralFlags ?? [],
        notes: parsed.data.notes ?? null,
        guardianRelationship:
          (body.guardianRelationship as string | null) ?? null,
    ...
    const initialPaymentAmount =
      typeof body.initialPaymentAmount === "number"
        ? body.initialPaymentAmount
        : parseFloat((body.initialPaymentAmount as string) || "0") || 0;
    const initialDiscount =
      typeof body.discount === "number"
        ? body.discount
        : parseFloat((body.discount as string) || "0") || 0;
      ...
      try {
        ...
        if (levelId) {
          const [level] = await db
            .select({ price: levelsTable.price })
            .from(levelsTable)
    ...
    const createParentAccount =
      body.createParentAccount === true || body.createParentAccount === "true";
    ...
    res
      .status(201)
      .json({ ...student, createdAt: student.createdAt.toISOString() });
  },
);

router.get(
  "/students/behavioral-feed",
  requireAuth,
  async (req: Request, res: Response): Promise<void> => {
    const user = (req as any).user;
    if (!["admin", "psychologist"].includes(user.role)) {
      res
        .status(403)
        .json({ error: "Access restricted to psychologist and admin." });
      return;
    }
    ...
    const flagCounts = await db
      ...
      .where(
        or(
          eq(observationsTable.observationType, "fear"),
          eq(observationsTable.observationType, "shyness"),
        ),
      )
    ...
    const countMap = new Map(
      flagCounts.map((r) => [r.studentId, Number(r.redFlagCount)]),
    );
    ...
    for (const obs of latestObs) {
      if (!latestObsMap.has(obs.studentId))
        latestObsMap.set(obs.studentId, obs);
    }
    ...
    const result = students
      .map((s) => ({
        ...
        latestObservation: latestObsMap.has(s.id)
          ? {
              ...latestObsMap.get(s.id)!,
              createdAt: latestObsMap.get(s.id)!.createdAt.toISOString(),
            }
          : null,
      }))
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (student.levelId) {
      const [level] = await db
        ...
        .from(levelsTable)
        .where(eq(levelsTable.id, student.levelId));
    ...
    if (student.teacherId) {
      const [teacher] = await db
        ...
        .from(usersTable)
        .where(eq(usersTable.id, student.teacherId));
    ...
    const requestingUser = (req as any).user as
      | { role: string; id: number }
      | undefined;
    const canViewPayments =
      requestingUser &&
      ["admin", "accountant", "parent"].includes(requestingUser.role);
    const canViewPrivateTip =
      requestingUser &&
      (["admin", "psychologist"].includes(requestingUser.role) ||
        (requestingUser.role === "teacher" &&
          student.teacherId === requestingUser.id));
...
router.put(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (parsed.data.levelId !== undefined)
      updateData.levelId = parsed.data.levelId;
    if (parsed.data.parentId !== undefined)
      updateData.parentId = parsed.data.parentId;
    if (parsed.data.teacherId !== undefined)
      updateData.teacherId = parsed.data.teacherId;
    if (parsed.data.enrollmentDate !== undefined)
      updateData.enrollmentDate = parsed.data.enrollmentDate;
    if (parsed.data.behavioralFlags !== undefined)
      updateData.behavioralFlags = parsed.data.behavioralFlags;
    ...
    if (body.guardianRelationship !== undefined)
      updateData.guardianRelationship = body.guardianRelationship || null;
    if (body.guardianName !== undefined)
      updateData.guardianName = body.guardianName || null;
    if (body.guardianPhone !== undefined)
      updateData.guardianPhone = body.guardianPhone || null;
    if (body.guardianPhone2 !== undefined)
      updateData.guardianPhone2 = body.guardianPhone2 || null;
    if (body.guardianOccupation !== undefined)
      updateData.guardianOccupation = body.guardianOccupation || null;
    if (body.medicalAlerts !== undefined)
      updateData.medicalAlerts = body.medicalAlerts || null;
    if (body.referralSource !== undefined)
      updateData.referralSource = body.referralSource || null;
    if (body.medicalIssues !== undefined)
      updateData.medicalIssues = body.medicalIssues;
    if (body.learningDisabilities !== undefined)
      updateData.learningDisabilities = body.learningDisabilities;
    if (body.supportInstructions !== undefined)
      updateData.supportInstructions = body.supportInstructions;
    if (body.preferredTeachingMethod !== undefined)
      updateData.preferredTeachingMethod = body.preferredTeachingMethod;

    if (
      body.status !== undefined &&
      ["active", "stopped", "graduated"].includes(body.status as string)
    ) {
    ...
    if (body.privateTip !== undefined) {
      if (!["admin", "psychologist"].includes(sessionUser.role)) {
        res
          .status(403)
          .json({
            error: "Only psychologists and admins can update the private tip.",
          });
        return;
      }
      updateData.privateTip =
        typeof body.privateTip === "string" ? body.privateTip : null;
      updateData.privateTipUpdatedBy = sessionUser.id;
    }

    updateData.lastUpdatedBy =
      sessionUser.name || sessionUser.email || String(sessionUser.id);
    ...
    const newFlags: string[] = Array.isArray(updateData.behavioralFlags)
      ? (updateData.behavioralFlags as string[])
      : currentStudent.behavioralFlags;
    const oldFlags = currentStudent.behavioralFlags;
    const isNewlyConfident =
      newFlags.includes(CONFIDENT_FLAG) && !oldFlags.includes(CONFIDENT_FLAG);
...
router.delete(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    if (!["admin", "branch_manager"].includes(caller?.role)) {
      res.status(403).json({ error: "Only admins can delete students." });
      return;
    }
    const params = GetStudentParams.safeParse(req.params);
    if (!params.success) {
    ...
    try {
      const [student] = await db
        ...
        .set({
          ...
          lastUpdatedBy: caller.name || caller.email || String(caller.id),
        })
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    const overallProgressScore = calcProgressScore(
      evals.map((e) => ({
        speakingScore: e.speakingScore,
        confidenceScore: e.confidenceScore,
        participationScore: e.participationScore,
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    ...
    // ── 1. Get daysPerWeek from the student's current group ─────────────────
    let daysPerWeek = 2;
    const [groupStudent] = await db
      .select({ groupId: groupStudentsTable.groupId })
      .from(groupStudentsTable)
      .where(eq(groupStudentsTable.studentId, id));

    if (groupStudent) {
      const [group] = await db
        .select({ recurringDays: groupsTable.recurringDays })
        .from(groupsTable)
        .where(eq(groupsTable.id, groupStudent.groupId));

      if (group?.recurringDays) {
        try {
          const days = JSON.parse(group.recurringDays) as number[];
          if (Array.isArray(days) && days.length > 0) daysPerWeek = days.length;
        } catch {
          /* keep default 2 */
        }
      }
    ...
    // SESSIONS_PER_LEVEL = actual days per week × 8 weeks (not hardcoded 16)
    const SESSIONS_PER_LEVEL = daysPerWeek * 8;

    // ── 2. Fetch all attended sessions with the group's level ────────────────
    const attendanceRows = await db
      ...
      .where(
        and(
          eq(sessionAttendanceTable.studentId, id),
          eq(sessionAttendanceTable.status, "present"),
        ),
      )
      .orderBy(classSessionsTable.sessionDate);

    // ── 3. Correct progress calculation ──────────────────────────────────────
    const completedTotal = attendanceRows.length;

    // Count only sessions in the student's CURRENT level (not modulo trick)
    const completedInLevel = student.levelId
      ? attendanceRows.filter((r) => r.groupLevelId === student.levelId).length
      : completedTotal;

    const currentWeek = Math.min(
      Math.floor(completedInLevel / daysPerWeek) + 1,
      8,
    );
    const currentSessionInWeek = (completedInLevel % daysPerWeek) + 1;
    const percentComplete = Math.min(
      100,
      Math.round((completedInLevel / SESSIONS_PER_LEVEL) * 100),
    );

    // ── 4. Skill averages (last 10 scored sessions) ───────────────────────────
    const scoredRows = attendanceRows
      .filter(
        (r) =>
          r.speakingScore != null ||
          r.confidenceScore != null ||
          r.participationScore != null ||
          r.initiativeScore != null,
      )
      .slice(-10);
    ...
    // ── 5. Recent sessions for timeline (last 6) ─────────────────────────────
    const recentSessions = attendanceRows
      .slice(-6)
      .reverse()
      .map((r) => ({
...
router.get(
  ...
  async (req: Request, res: Response) => {
    ...
    if (me.role === "parent") {
      ...
      if (!student || student.parentId !== me.id)
    ...
    const psychologicalGroups = await db
      ...
      .innerJoin(
        programsTable,
        and(
          eq(programsTable.id, levelsTable.programId),
          eq(programsTable.type, "psychological"),
        ),
      )
    ...
    const allGroupNameMap = Object.fromEntries(
      allStudentGroups.map((g) => [g.groupId, g.groupName]),
    );
    ...
    if (psychologicalGroups.length > 0) {
      ...
      const groupNameMap = Object.fromEntries(
        psychologicalGroups.map((g) => [g.groupId, g.groupName]),
      );
      const rows = await db
        .select({
...
router.get(
  "/students/:id/communication-metrics",
  requireAuth,
  async (req: Request, res: Response): Promise<void> => {
    const me = (req as any).user;
    const studentId = parseInt(req.params.id as string);
    ...
    const rows = await db
      ...
      .innerJoin(
        classSessionsTable,
        eq(classSessionsTable.id, sessionAttendanceTable.sessionId),
      )
      .where(eq(sessionAttendanceTable.studentId, studentId))
      .orderBy(desc(classSessionsTable.sessionDate));

    const withComm = rows.filter(
      (r) =>
        r.verbalFluency != null ||
        r.verbalClarity != null ||
        r.verbalVocabulary != null ||
        r.nonverbalEyeContact != null ||
        r.nonverbalBodyLanguage != null ||
        r.nonverbalFacialExpressions != null,
    );
