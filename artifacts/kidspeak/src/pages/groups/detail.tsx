...
export default function GroupDetail() {
  ...
  // ── Session tab ──────────────────────────────────────────────────────────
  const [sessionTab, setSessionTab] = useState<"academic" | "psychological" | "reports">("academic");
        ...
        <div className="lg:col-span-2">
          <Card>
            <CardHeader>
              ...
              {/* ── Tabs: academic/psych for admins with psych level; Reports for all admins ── */}
              {isAdmin && (
                <div className="flex gap-1 mt-2 flex-wrap">
                  ...
                  <button
                    onClick={() => setSessionTab("reports")}
                    className={`px-3 py-1 rounded-full text-xs font-semibold transition-all ${sessionTab === "reports" ? "text-white" : "text-muted-foreground bg-muted hover:bg-muted/80"}`}
                    style={sessionTab === "reports" ? { backgroundColor: "#16a34a" } : {}}
                  >
                    <ClipboardList className="w-3 h-3 inline me-1" />
                    {(t as any).groups?.reports ?? "التقارير"}
                  </button>
                </div>
              )}
            </CardHeader>
            <CardContent>
              {sessionTab === "reports" && isAdmin ? (
                (() => {
                  const reports = (group.sessions as any[]).filter((s: any) => s.reportStatus === "published");
                  if (reports.length === 0) {
                    return (
                      <div className="text-center py-12 text-muted-foreground">
                        <ClipboardList className="w-12 h-12 mx-auto mb-3 opacity-20" />
                        <p className="font-medium">{isAr ? "لا توجد تقارير منشورة" : "No published reports"}</p>
                      </div>
                    );
                  }
                  return (
                    <div className="grid gap-3 sm:grid-cols-2">
                      {reports.map((s: any) => (
                        <div key={s.id} className="rounded-xl border bg-card p-4 hover:shadow-md transition-shadow flex flex-col gap-2">
                          ...
                          {s.lessonTitle && (
                            <div className="font-semibold text-sm line-clamp-2">{s.lessonTitle}</div>
                          )}
                          {s.teacherName && (
                            <div className="text-xs text-muted-foreground">
                              {isAr ? "الأستاذ:" : "Teacher:"} {s.teacherName}
                            </div>
                          )}
                          <Button
                            size="sm"
                            ...
                            onClick={() => navigate(`/groups/${groupId}/sessions/${s.id}/report`)}
                          >
