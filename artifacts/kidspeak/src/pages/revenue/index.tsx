...
import { Plus, ArrowUpRight, ArrowDownRight, TrendingUp, TrendingDown, Receipt as ReceiptIcon, Minus, Equal, Pencil, Trash2, Wallet, Users as UsersIcon, Scale } from "lucide-react";
...
export default function RevenueDashboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      <StaffEarningsSummary month={month} isRTL={isRTL} t={t} />
...
function StaffEarningsSummary({ month, isRTL, t }: { month: string; isRTL: boolean; t: any }) {
  const { data, isLoading } = useQuery<{ totalEarned: number; totalPaid: number; balance: number }>({
    queryKey: ["earnings", "summary", month],
    queryFn: async () => {
      const r = await fetch(`/api/earnings/summary?month=${encodeURIComponent(month)}`, {
        credentials: "include",
      });
      if (!r.ok) return { totalEarned: 0, totalPaid: 0, balance: 0 };
      return r.json();
    },
  });
  ...
  return (
    <Card className="overflow-hidden shadow-md border-0">
      <div className="px-6 py-3 flex items-center gap-2" style={{ backgroundColor: BRAND_BLUE }}>
        <UsersIcon className="w-4 h-4" style={{ color: BRAND_YELLOW }} />
        <span className="text-white font-bold text-sm uppercase tracking-wider">
          {isRTL ? "ملخص رواتب الموظفين" : "Staff Earnings Summary"}
        </span>
        <span className="ms-auto text-white/60 text-xs">{month}</span>
      </div>
      <CardContent className="p-0">
        <div className="grid md:grid-cols-3 divide-y md:divide-y-0 md:divide-x">
          <div className="flex items-center gap-4 px-6 py-5">
            ...
            <div>
              <p className="text-xs text-muted-foreground">
                {isRTL ? "إجمالي المستحقات" : "Total Earned"}
              </p>
              <p className="text-xl font-bold text-blue-700">
                {isLoading ? "—" : t.currency.format(earned)}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-4 px-6 py-5">
            ...
            <div>
              ...
              <p className="text-xl font-bold text-emerald-700">
                {isLoading ? "—" : t.currency.format(paid)}
              </p>
            </div>
          </div>
          <div className="flex items-center gap-4 px-6 py-5">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${balance > 0 ? "bg-amber-100" : "bg-muted"}`}>
              <Scale className={`w-5 h-5 ${balance > 0 ? "text-amber-700" : "text-muted-foreground"}`} />
            </div>
            <div>
              <p className="text-xs text-muted-foreground">
                {isRTL ? "الرصيد المتبقي" : "Outstanding Balance"}
              </p>
              <p className={`text-xl font-bold ${balance > 0 ? "text-amber-700" : "text-foreground"}`}>
                {isLoading ? "—" : t.currency.format(balance)}
              </p>
            </div>
          </div>
        </div>
