...
interface SavedListDetail extends SavedList {
  members: Array<{ userId: number; name: string; role: string; email: string | null }>;
}
...
export default function InboxPage() {
  ...
  const [bListId, setBListId] = useState<string>("");

  // ── Saved Broadcast Lists (BUG 8) ────────────────────────────────────────
  const [listsManagerOpen, setListsManagerOpen] = useState(false);
  const [editingList, setEditingList] = useState<SavedListDetail | null>(null);
  const [listFormName, setListFormName] = useState("");
  const [listFormDescription, setListFormDescription] = useState("");
  const [listFormMemberIds, setListFormMemberIds] = useState<Set<number>>(new Set());
  ...
  const refreshSavedLists = useCallback(async () => {
    try {
      const r = await fetch("/api/messages/lists", { credentials: "include" });
      if (r.ok) setSavedLists(await r.json());
    } catch {}
  }, []);

  // Auto-load saved lists for admins so the broadcast "list" picker is populated
  // even when the user hasn't opened the manager dialog first.
  useEffect(() => {
    if (broadcastOpen && (window as any).__kidspeakIsAdmin !== false) {
      refreshSavedLists();
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [broadcastOpen]);

  // ── Read Receipts (BUG 10) ───────────────────────────────────────────────
  const [receiptsOpen, setReceiptsOpen] = useState(false);
  const [receiptsBatchId, setReceiptsBatchId] = useState<string | null>(null);
  const [receiptsData, setReceiptsData] = useState<{
    recipients: Array<{ userId: number; name: string; role: string; isRead: boolean; readAt: string | null }>;
    totalRead: number;
    totalRecipients: number;
  } | null>(null);
  const [isLoadingReceipts, setIsLoadingReceipts] = useState(false);

  const openReceipts = useCallback(async (batchId: string) => {
    setReceiptsBatchId(batchId);
    setReceiptsOpen(true);
    setIsLoadingReceipts(true);
    setReceiptsData(null);
    try {
      const r = await fetch(`/api/messages/broadcasts/${encodeURIComponent(batchId)}/reads`, { credentials: "include" });
      if (r.ok) setReceiptsData(await r.json());
    } catch {} finally {
  ...
  const handleSendBroadcast = () => {
    ...
    else if (bType === "list") payload.listId = parseInt(bListId);
  ...
  return (
    <div className="h-[calc(100vh-8rem)] flex flex-col">
                            ...
                            <div className={`max-w-[75%] flex flex-col ${isMine ? "items-end" : "items-start"}`}>
                              ...
                              <div className="flex items-center gap-1 mt-0.5">
                                ...
                                {isMine && msg.batchId && (
                                  <button
                                    type="button"
                                    onClick={() => openReceipts(msg.batchId!)}
                                    className="text-xs text-[#1B2E8F] hover:underline ms-1 flex items-center gap-0.5"
                                    title={language === "ar" ? "عرض إيصالات القراءة" : "View read receipts"}
                                  >
                                    <Eye className="w-3 h-3" />
                                    {language === "ar" ? "إيصالات" : "Receipts"}
                                  </button>
                                )}
                              </div>
                            </div>
          ...
          <div className="space-y-4 py-2">
            ...
            <div className="space-y-1.5">
              ...
              <div className="flex flex-wrap gap-2">
                {(isAdmin
                  ? (["group", "level", "role", "list", "specific_students", "all_parents", "global"] as BroadcastType[])
                  ...
                  >
                    {bt === "specific_students"
                      ...
                      : bt === "list"
                      ? (language === "ar" ? "قائمة محفوظة" : "Saved List")
              ...
              {isAdmin && (
                <button
                  onClick={() => { refreshSavedLists(); setListsManagerOpen(true); }}
                  className="text-xs text-[#1B2E8F] hover:underline mt-1"
                >
                  {language === "ar" ? "إدارة القوائم المحفوظة ⚙" : "Manage saved lists ⚙"}
                </button>
              )}
            </div>
            ...
            {/* Saved List picker */}
            {bType === "list" && (
              <Select value={bListId} onValueChange={setBListId}>
                <SelectTrigger>
                  <SelectValue
                    placeholder={savedLists.length === 0
                      ? (language === "ar" ? "لا توجد قوائم محفوظة" : "No saved lists yet")
                      : (language === "ar" ? "اختر قائمة" : "Select a list")}
                  />
                </SelectTrigger>
                <SelectContent>
                  {savedLists.length === 0 ? (
            ...
            {bType === "group" && (
              <Select value={bGroupId} onValueChange={setBGroupId}>
                <SelectTrigger>
                  <SelectValue placeholder={groupOptions.length === 0 ? (language === "ar" ? "لا توجد أفواج" : "No groups available") : mt.selectGroup} />
                </SelectTrigger>
                <SelectContent>
                  {groupOptions.length === 0 ? (
                    <SelectItem value="_empty" disabled>
                      {language === "ar" ? "لا توجد أفواج متاحة" : "No groups available"}
                    </SelectItem>
                  ) : (
                    groupOptions.map((g) => (
            ...
            {bType === "level" && (
              <Select value={bLevelId} onValueChange={setBLevelId}>
                <SelectTrigger>
                  <SelectValue placeholder={levels.length === 0 ? (language === "ar" ? "لا توجد مستويات" : "No levels available") : mt.selectLevel} />
                </SelectTrigger>
                <SelectContent>
                  {levels.length === 0 ? (
                    <SelectItem value="_empty" disabled>
      ...
      {/* ── Saved Lists Manager Dialog (BUG 8) ──────────────────────────── */}
      <Dialog
        open={listsManagerOpen}
        onOpenChange={(o) => {
          setListsManagerOpen(o);
          if (!o) {
            setEditingList(null);
            setListFormName("");
            setListFormDescription("");
            setListFormMemberIds(new Set());
            setListSearch("");
          }
        }}
      >
        <DialogContent className="sm:max-w-2xl">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Users className="w-5 h-5" style={{ color: "#1B2E8F" }} />
              {language === "ar" ? "القوائم المحفوظة" : "Saved Lists"}
            </DialogTitle>
          </DialogHeader>
            ...
            <div className="space-y-3 py-2">
              <div className="flex items-center justify-between">
                <p className="text-sm text-muted-foreground">
                  {language === "ar" ? "قوائم مخصصة من المستلمين لإعادة استخدامها." : "Custom recipient lists you can reuse."}
                </p>
                <Button
                  size="sm"
                  style={{ backgroundColor: "#1B2E8F", color: "#fff" }}
                  onClick={() => {
                    setEditingList({ id: 0, name: "", description: null, memberCount: 0, createdAt: "", members: [] });
                    setListFormName("");
                    setListFormDescription("");
                    setListFormMemberIds(new Set());
                    setListSearch("");
              ...
              <div className="border rounded-lg divide-y max-h-[50vh] overflow-y-auto">
                {savedLists.length === 0 ? (
                  <p className="text-sm text-muted-foreground text-center py-8">
                    {language === "ar" ? "لا توجد قوائم محفوظة بعد." : "No saved lists yet."}
                  </p>
                ) : (
                  savedLists.map((l) => (
                    <div key={l.id} className="flex items-center justify-between p-3">
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {l.memberCount} {language === "ar" ? "مستلم" : "recipients"}
                      ...
                      <div className="flex items-center gap-1">
                        <Button
                          ...
                          onClick={async () => {
                            try {
                              const r = await fetch(`/api/messages/lists/${l.id}`, { credentials: "include" });
                              if (r.ok) {
                                const detail: SavedListDetail = await r.json();
                                setEditingList(detail);
                                setListFormName(detail.name);
                                setListFormDescription(detail.description ?? "");
                                setListFormMemberIds(new Set(detail.members.map((m) => m.userId)));
                                setListSearch("");
                              }
                            } catch {}
                          }}
                        ...
                        <Button
                          ...
                          onClick={async () => {
                            ...
                            try {
                              const r = await fetch(`/api/messages/lists/${l.id}`, {
                                method: "DELETE",
                                credentials: "include",
                              });
                              if (r.ok) {
                                if (String(l.id) === bListId) setBListId("");
                                refreshSavedLists();
                              }
                            } catch {}
                          }}
              ...
              <DialogFooter>
                <DialogClose asChild>
                  <Button variant="outline">{t.groups?.cancel ?? "Close"}</Button>
                </DialogClose>
              </DialogFooter>
            </div>
          ) : (
            <div className="space-y-3 py-2">
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  placeholder={language === "ar" ? "مثال: أولياء أمور المستوى الثالث" : "e.g. Level 3 parents"}
                />
              </div>
              <div className="space-y-1.5">
                <label className="text-sm font-medium">
              ...
              <div className="space-y-1.5">
                <label className="text-sm font-medium">
                  {language === "ar" ? "أعضاء القائمة" : "Members"} ({listFormMemberIds.size})
                </label>
                <Input
                ...
                <div className="border rounded-lg max-h-[40vh] overflow-y-auto divide-y">
                  {contacts
                    ...
                    .map((c) => {
                      const checked = listFormMemberIds.has(c.id);
                      return (
                        <label
                          key={c.id}
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <span
                              className="text-xs px-1.5 py-0.5 rounded font-medium"
                              style={{
                                backgroundColor: `${ROLE_COLORS[c.role] ?? "#1B2E8F"}22`,
                                color: ROLE_COLORS[c.role] ?? "#1B2E8F",
                              }}
                            >
                              {roleLabel(c.role)}
                            </span>
              ...
              <DialogFooter className="gap-2">
                <Button variant="outline" onClick={() => setEditingList(null)} disabled={isSavingList}>
                  {t.groups?.cancel ?? "Cancel"}
                </Button>
                <Button
                  style={{ backgroundColor: "#1B2E8F", color: "#fff" }}
                  disabled={isSavingList || !listFormName.trim()}
                  onClick={async () => {
                    setIsSavingList(true);
                    try {
                      const isNew = !editingList || editingList.id === 0;
                      const url = isNew ? "/api/messages/lists" : `/api/messages/lists/${editingList!.id}`;
                      ...
                      const r = await fetch(url, {
                        ...
                        body: JSON.stringify({
                          ...
                          description: listFormDescription.trim(),
                          memberIds: Array.from(listFormMemberIds),
                        }),
                      });
                      if (r.ok) {
      ...
      {/* ── Read Receipts Dialog (BUG 10) ───────────────────────────────── */}
      <Dialog open={receiptsOpen} onOpenChange={setReceiptsOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              <Eye className="w-5 h-5" style={{ color: "#1B2E8F" }} />
              {language === "ar" ? "إيصالات القراءة" : "Read Receipts"}
            </DialogTitle>
          </DialogHeader>
          <div className="py-2">
            {isLoadingReceipts ? (
              ...
              <p className="text-sm text-muted-foreground text-center py-6">
                {language === "ar" ? "تعذر التحميل." : "Unable to load."}
              </p>
            ) : (
              <>
                <div className="flex items-center justify-between mb-3 px-1">
                  <span className="text-sm font-medium">
                    {language === "ar"
                      ? `${receiptsData.totalRead} من ${receiptsData.totalRecipients} قرأ`
                      : `${receiptsData.totalRead} of ${receiptsData.totalRecipients} read`}
                  </span>
                  <span className="text-xs text-muted-foreground">
                    {receiptsData.totalRecipients > 0
                      ? `${Math.round((receiptsData.totalRead / receiptsData.totalRecipients) * 100)}%`
                      : "—"}
                  </span>
                </div>
                <div className="border rounded-lg divide-y max-h-[50vh] overflow-y-auto">
                  {receiptsData.recipients.map((r) => (
                    <div key={r.userId} className="flex items-center gap-3 p-2.5">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <span
                          className="text-xs px-1.5 py-0.5 rounded font-medium"
                          style={{
                            backgroundColor: `${ROLE_COLORS[r.role] ?? "#1B2E8F"}22`,
                            color: ROLE_COLORS[r.role] ?? "#1B2E8F",
                          }}
                        >
                          {roleLabel(r.role)}
                        </span>
                      ...
                      <div className="text-xs">
                        {r.isRead ? (
                          <div className="flex items-center gap-1 text-[#1B2E8F]">
                            <CheckCheck className="w-3.5 h-3.5" />
                            <span>{r.readAt ? safeFmt(r.readAt, "MMM d, HH:mm") : "✓"}</span>
                          </div>
                        ) : (
                          <span className="text-muted-foreground">
                            {language === "ar" ? "لم يقرأ" : "Unread"}
                          </span>
                        )}
                      </div>
                    </div>
