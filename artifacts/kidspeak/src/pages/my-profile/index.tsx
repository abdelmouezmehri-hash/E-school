...
function MySalarySection({ isRTL }: { isRTL: boolean }) {
  const { data, isLoading } = useQuery<{ salaries: any[]; summary: any | null }>({
    ...
    queryFn: async () => {
      try {
        ...
        if (Array.isArray(json)) return { salaries: json, summary: null };
        return { salaries: json?.salaries ?? [], summary: json?.summary ?? null };
      } catch {
...
export default function MyProfile() {
  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto pb-10">
      ...
      {/* My Salary Section — only for staff, NOT admin or parent */}
